#!/usr/bin/env bash
#
# Run smoke tests for each pair in flyover-lps-api-e2e/compat/lps-lbc-matrix.yaml.
# Green = all smokes pass, orange = some pass, red = none pass.
#
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PAIR_SCRIPT="${SCRIPT_DIR}/run-pair.sh"
LPS_REPO="${LPS_REPO:-$(cd "${SCRIPT_DIR}/../../.." && pwd)}"
...
run_cell() {
  ...
  set +e
  E2E_REPO="$E2E_REPO" LPS_REPO="$LPS_REPO" \
    "$PAIR_SCRIPT" --clean-before-up --lps-ref "$lps_ref" --lbc-ref "$lbc_ref" 2>&1 | tee "$cell_log"
...
echo "=== LPS x LBC smoke matrix (${TOTAL_CELLS} pairs from flyover-lps-api-e2e) ==="
...
(
  cd "$E2E_REPO"
  npx --yes tsx scripts/print-compat-matrix.ts --run-results "$RESULTS_FILE"
)
