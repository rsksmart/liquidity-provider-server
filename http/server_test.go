...
func testAcceptQuoteComplete(t *testing.T) {
	for _, quote := range testQuotes {
		...
		btc.On("AddAddressWatcher", "2N6LWQUrCPwexzzDTrBa1fvx1ZXn1YNXtqU", minAmount, time.Minute, expTime, mock.AnythingOfType("*http.BTCAddressWatcher"), mock.AnythingOfType("func(connectors.AddressWatcher)")).Times(1).Return("")
