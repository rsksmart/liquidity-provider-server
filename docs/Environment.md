...
| `PEGIN_ADDRESS_REGISTRY_WATCHER_START_BLOCK` | Block from which the pegin address registry discovery watchers start scanning. Must be set together with `PEGIN_ADDRESS_REGISTRY_WATCHER_PAGE_SIZE` to enable the watchers; leaving either unset keeps them off. | `500` | NO |
| `PEGIN_ADDRESS_REGISTRY_WATCHER_PAGE_SIZE` | Number of blocks scanned per page by the pegin address registry discovery watchers. Must be greater than zero when set, and set together with `PEGIN_ADDRESS_REGISTRY_WATCHER_START_BLOCK`. | `1000` | NO |
