...
This is phase 1 of a planned three-phase rollout (see [FLY-2292][jira]).
...
An earlier version of this lived in `scripts/check-release-drift.sh`. It was
ported to Node for **readability and maintainability**:

- Per-commit data flows as plain objects (`{ sha, author, date, subject,
  releasedIn, pr }`) instead of being repeatedly split out of TAB-separated
  strings.
- One semver-aware comparator replaces two near-duplicate bash helpers that
  worked around `git tag --sort=v:refname` ordering pre-releases after stable.
- GitHub Release enrichment uses built-in `JSON.parse` — the script no longer
  depends on `jq`.
- Report rendering is a series of template literals and `Array.join`s rather
  than ~100 `echo` lines, so adding new columns or a JSON output mode is a
  small change.

Every `git` invocation, flag, and pretty-format string is unchanged from the
bash version, so the report contents are byte-for-byte equivalent on the same
repo state (modulo cosmetic tie-breaks in author sort order).
...
| --- | --- | --- |
| `RELEASE_BRANCH_PATTERN` | `v[0-9]*.[0-9]*.[0-9]*` | Git glob used against `refs/remotes/$REMOTE/`. Anything matching is treated as a release branch and sorted by `sort -V`. |
...
```bash
RELEASE_BRANCH_PATTERN='release/*' TRUNK_BRANCH=main \
  node scripts/check-release-drift.mjs
```
...
[jira]: https://rsklabs.atlassian.net/browse/FLY-2292
