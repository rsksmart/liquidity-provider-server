...
func TestGetPegoutCollateralHandlerHappyPath(t *testing.T) {
	request := httptest.NewRequest(http.MethodGet, "/pegout/collateral", nil)
	recorder := httptest.NewRecorder()

	expectedCollateral := entities.NewWei(5000000000000000000)

	mockUseCase := new(mocks.GetPegoutCollateralUseCaseMock)
	mockUseCase.On("Run").Return(expectedCollateral, nil)

	handlerFunc := handlers.NewGetPegoutCollateralHandler(mockUseCase)
...
func TestGetPegoutCollateralHandlerErrorCases(t *testing.T) {
...
func TestGetPegoutCollateralHandlerErrorResponseFormat(t *testing.T) {
