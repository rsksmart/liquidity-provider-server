...
// nolint:funlen
func TestNewGetReportsPegoutHandler(t *testing.T) {
	type testCase struct {
		name      string
		startDate string
		endDate   string
		mockSetup func(mock *mocks.GetPegoutReportUseCaseMock)
		result    int
	}
	...
	tests := []testCase{
		...
		},
		{
			...
			startDate: "2024-01-01",
			endDate:   "2025-08-27",
			mockSetup: func(useCase *mocks.GetPegoutReportUseCaseMock) {
				useCase.EXPECT().
					Run(mock.Anything, mock.Anything, mock.Anything).
					Return(reports.GetPegoutReportResult{}, assert.AnError).
					Once()
			},
		...
		},
		{
			...
			mockSetup: func(useCase *mocks.GetPegoutReportUseCaseMock) {
				useCase.EXPECT().
					Run(mock.Anything, mock.Anything, mock.Anything).
					Return(reports.GetPegoutReportResult{
						NumberOfQuotes:     1,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			useCase := &mocks.GetPegoutReportUseCaseMock{}
			tc.mockSetup(useCase)

			req, err := http.NewRequestWithContext(context.Background(), http.MethodGet, "/reports/pegout", nil)
			...
			handler := handlers.NewGetReportsPegoutHandler(handlerSingleFlight, groupKey, useCase)
...
// nolint:funlen
func TestNewGetReportsPegoutHandler_Concurrency(t *testing.T) {
	t.Run("should only call use case once for concurrent requests", func(t *testing.T) {
		useCase := &mocks.GetPegoutReportUseCaseMock{}
		const groupKey = "test-key"
		handlerSingleFlight := new(singleflight.Group)
		result := reports.GetPegoutReportResult{
			NumberOfQuotes:     1,
		...
		useCase.EXPECT().Run(mock.Anything, mock.Anything, mock.Anything).
			RunAndReturn(func(ctx context.Context, start time.Time, end time.Time) (reports.GetPegoutReportResult, error) {
		...
		results := make(map[int]pkg.GetPegoutReportResponse, testSize)
		mutex := new(sync.Mutex)
		for i := 0; i < testSize; i++ {
			go func(requestId int, wg *sync.WaitGroup, resultMap map[int]pkg.GetPegoutReportResponse, mapMutex *sync.Mutex) {
				req, err := http.NewRequestWithContext(context.Background(), http.MethodGet, "/reports/pegout?startDate=2024-08-27&endDate=2025-08-27", nil)
				assert.NoError(t, err)
				rr := httptest.NewRecorder()
				handler.ServeHTTP(rr, req)
				assert.Equal(t, http.StatusOK, rr.Code)
				var body pkg.GetPegoutReportResponse
		...
		expected := pkg.GetPegoutReportResponse{
			NumberOfQuotes:     1,
	...
	t.Run("should be capable of handling multiple groups", func(t *testing.T) {
		...
		const groupKey = "test-key"
		handlerSingleFlight := new(singleflight.Group)
		resultGroupOne := reports.GetPegoutReportResult{
			NumberOfQuotes:     1,
			MinimumQuoteValue:  entities.NewWei(2),
			MaximumQuoteValue:  entities.NewWei(3),
			AverageQuoteValue:  entities.NewWei(4),
		...
		resultGroupTwo := reports.GetPegoutReportResult{
			NumberOfQuotes:     7,
		...
		for i := 0; i < testSize; i++ {
			go func(requestId int, wg *sync.WaitGroup, resultMap map[int]pkg.GetPegoutReportResponse, mapMutex *sync.Mutex) {
				...
				if i%2 == 0 {
					req, err = http.NewRequestWithContext(context.Background(), http.MethodGet, fmt.Sprintf("/reports/pegout?startDate=%s&endDate=%s", groupOneStart, groupOneEnd), nil)
				} else {
					req, err = http.NewRequestWithContext(context.Background(), http.MethodGet, fmt.Sprintf("/reports/pegout?startDate=%s&endDate=%s", groupTwoStart, groupTwoEnd), nil)
				}
				assert.NoError(t, err)
				rr := httptest.NewRecorder()
				handler.ServeHTTP(rr, req)
		...
		expectedGroupOne := pkg.GetPegoutReportResponse{
			NumberOfQuotes:     1,
			MinimumQuoteValue:  big.NewInt(2),
			MaximumQuoteValue:  big.NewInt(3),
			AverageQuoteValue:  big.NewInt(4),
		...
		expectedGroupTwo := pkg.GetPegoutReportResponse{
			NumberOfQuotes:     7,
