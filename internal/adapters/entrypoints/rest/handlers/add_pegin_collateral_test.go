...
func TestAddPeginCollateralHandlerHappyPath(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/pegin/addCollateral", bytes.NewBuffer(jsonBody))
	request.Header.Set("Content-Type", "application/json")
	recorder := httptest.NewRecorder()

	expectedBalance := entities.NewWei(2000000000000000000)

	mockUseCase := new(mocks.AddPeginCollateralUseCaseMock)
	mockUseCase.On("Run", mock.AnythingOfType("*entities.Wei")).Return(expectedBalance, nil)

	handlerFunc := handlers.NewAddPeginCollateralHandler(mockUseCase)
...
// nolint:funlen
func TestAddPeginCollateralHandlerErrorCases(t *testing.T) {

	t.Run("should handle malformed JSON in request body", func(t *testing.T) {
		malformedJSON := []byte(`{"amount": "not-a-number"`)
		request := httptest.NewRequest(http.MethodPost, "/pegin/addCollateral", bytes.NewBuffer(malformedJSON))
		request.Header.Set("Content-Type", "application/json")
		recorder := httptest.NewRecorder()
...
// nolint:funlen
func TestAddPeginCollateralHandlerErrorResponseFormat(t *testing.T) {
