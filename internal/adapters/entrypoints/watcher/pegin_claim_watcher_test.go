...
func TestPegInClaimWatcher_DiscoveredRowIsIgnored(t *testing.T) {
	...
	discovered.State = rootstock.PegInAddressRegistryWatchDiscovered
	watchRepo.EXPECT().List(mock.Anything).Return([]rootstock.PegInAddressRegistryWatchEntry{discovered}, nil).Once()
