...
func (watcher *PegInAddressRegistryMetricsWatcher) Start() {
	...
	for {
		select {
		case event := <-mismatchEvents:
			if _, ok := event.(blockchain.PegInAddressRegistryRootMismatchEvent); ok {
				watcher.appMetrics.IncrementPegInAddressRegistryRootMismatch()
			}
		case event := <-resyncEvents:
			if _, ok := event.(blockchain.PegInAddressRegistryResyncStartedEvent); ok {
