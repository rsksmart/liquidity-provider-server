...
func TestUpdatePeginDepositUseCase_Run(t *testing.T) {
	...
	watchedPeginQuote, err := useCase.Run(context.Background(), quote.NewWatchedPeginQuote(peginQuote, retainedQuote, creationData), block, tx)
...
func TestUpdatePeginDepositUseCase_Run_ErrorHandling(t *testing.T) {
	...
	t.Run("Fail by illegal quote state", func(t *testing.T) {
		states := []quote.PeginState{
			quote.PeginStateWaitingForDepositConfirmations, quote.PeginStateTimeForDepositElapsed, quote.PeginStateCallForUserSucceeded,
			quote.PeginStateCallForUserFailed, quote.PeginStateRegisterPegInSucceeded, quote.PeginStateRegisterPegInFailed,
		}
		quoteRepository := new(mocks.PeginQuoteRepositoryMock)
