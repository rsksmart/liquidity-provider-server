...
) (watch *rootstock.PegInAddressRegistryWatch, needsRescan bool, err error) {
	watch, err = useCase.loadOrCreate(ctx, event)
	if err != nil {
		return nil, false, err
	}
	if watch.State == rootstock.PegInAddressRegistryWatchImported ||
...
func (useCase *DiscoverRegisteredAddressUseCase) loadOrCreate(
	...
	created := rootstock.NewPegInAddressRegistryWatch(
		event.TxHash,
		event.LogIndex,
		event.BlockNumber,
		event.RskAddress,
