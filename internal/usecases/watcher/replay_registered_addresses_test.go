...
func newReplayUseCase(
	...
	t.Helper()
	discover := watcher.NewDiscoverRegisteredAddressUseCase(repository, registry, mocks.NewBitcoinWalletMock(t))
	getWatched := watcher.NewGetWatchedRegisteredAddressesUseCase(repository)
...
func TestNewReplayRegisteredAddressesUseCase(t *testing.T) {
	require.NotNil(t, newReplayHarness(t).useCase(100, 10, 2))
}
...
func TestReplayRegisteredAddressesUseCase_Run_DoesNotQueryBeforeConfiguredStart(t *testing.T) {
	...
	pending, err := harness.useCase(startBlock, 10, finalityDepth).Run(context.Background(), false)
...
func TestReplayRegisteredAddressesUseCase_Run_WrapsErrors(t *testing.T) {
	...
	_, err := harness.useCase(100, 10, 0).Run(context.Background(), false)
...
func TestReplayRegisteredAddressesUseCase_Run_DeletesCheckpointWhenFinalizedHeadIsBelowStart(t *testing.T) {
	...
	pending, err := harness.useCase(startBlock, 10, finalityDepth).Run(context.Background(), true)
...
func TestReplayRegisteredAddressesUseCase_Run_SecondRunWaitsUntilFirstRunFinishes(t *testing.T) {
	...
	useCase := harness.useCase(0, 1, 0)

	firstResult := make(chan error, 1)
	go func() {
		_, err := useCase.Run(context.Background(), false)
		firstResult <- err
	}()
	select {
...
func TestReplayRegisteredAddressesUseCase_Run_DoesNotPublishIncrementalReplayBeforeRootVerification(t *testing.T) {
	...
	_, err := harness.useCase(0, 10, 0).Run(context.Background(), false)
...
func TestReplayRegisteredAddressesUseCase_ValidateDeployment_RejectsStartBlockThatIsNotDeployment(t *testing.T) {
	registry := mocks.NewPegInAddressRegistryContractMock(t)
	proofCalled := false
	proof := &replayDeploymentProof{
		PegInAddressRegistryContract: registry,
	...
	useCase := newReplayUseCase(t, newReplayCheckpointRepository(t), proof, mocks.NewRootstockRpcServerMock(t), 100, 10, 0)

	err := useCase.ValidateDeployment(context.Background(), 100)
...
func TestReplayRegisteredAddressesUseCase_ValidateDeployment_WrapsIncompatibleDeployment(t *testing.T) {
	...
	registry.EXPECT().
		...
		Once()
	useCase := newReplayUseCase(
		t,
		newReplayCheckpointRepository(t),
		alwaysDeploymentBlock(registry),
		mocks.NewRootstockRpcServerMock(t),
		100,
		10,
		0,
	)
...
func TestReplayRegisteredAddressesUseCase_ValidateDeployment_RejectsMismatchedDeploymentRoot(t *testing.T) {
	registry := mocks.NewPegInAddressRegistryContractMock(t)
	registry.EXPECT().
		GetRegistrationRoot(mock.Anything, uint64(100)).
...
func TestReplayRegisteredAddressesUseCase_ValidateDeployment_AllowsFirstEventOnDeploymentBlock(t *testing.T) {
	...
	require.NoError(t, useCase.ValidateDeployment(context.Background(), 100))
}

func alwaysDeploymentBlock(contract blockchain.PegInAddressRegistryContract) blockchain.PegInAddressRegistryContract {
	return &replayDeploymentProof{
