...
import (
	...
	cold_wallet "github.com/rsksmart/liquidity-provider-server/internal/entities/cold_wallet"
...
// ColdWallet provides a mock function with given fields: rpc
func (_m *AbstractFactoryMock) ColdWallet(rpc blockchain.Rpc) (cold_wallet.ColdWallet, error) {
	ret := _m.Called(rpc)

	if len(ret) == 0 {
		panic("no return value specified for ColdWallet")
	}

	var r0 cold_wallet.ColdWallet
	var r1 error
	if rf, ok := ret.Get(0).(func(blockchain.Rpc) (cold_wallet.ColdWallet, error)); ok {
		return rf(rpc)
	}
	if rf, ok := ret.Get(0).(func(blockchain.Rpc) cold_wallet.ColdWallet); ok {
		r0 = rf(rpc)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(cold_wallet.ColdWallet)
		}
	}

	if rf, ok := ret.Get(1).(func(blockchain.Rpc) error); ok {
		r1 = rf(rpc)
	} else {
		r1 = ret.Error(1)
	}
...
// AbstractFactoryMock_ColdWallet_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'ColdWallet'
type AbstractFactoryMock_ColdWallet_Call struct {
	*mock.Call
}

// ColdWallet is a helper method to define mock.On call
//   - rpc blockchain.Rpc
func (_e *AbstractFactoryMock_Expecter) ColdWallet(rpc interface{}) *AbstractFactoryMock_ColdWallet_Call {
	return &AbstractFactoryMock_ColdWallet_Call{Call: _e.mock.On("ColdWallet", rpc)}
}

func (_c *AbstractFactoryMock_ColdWallet_Call) Run(run func(rpc blockchain.Rpc)) *AbstractFactoryMock_ColdWallet_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(blockchain.Rpc))
	})
	return _c
}

func (_c *AbstractFactoryMock_ColdWallet_Call) Return(_a0 cold_wallet.ColdWallet, _a1 error) *AbstractFactoryMock_ColdWallet_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *AbstractFactoryMock_ColdWallet_Call) RunAndReturn(run func(blockchain.Rpc) (cold_wallet.ColdWallet, error)) *AbstractFactoryMock_ColdWallet_Call {
	_c.Call.Return(run)
	return _c
}
